...
class AnalyzeContext {
	...
	Lexeme getNextLexeme() {
		// 从结果集取出，并移除第一个Lexme
		Lexeme result = this.results.peekFirst();
		while (result != null) {
			result = this.results.peekFirst();
			// 数量词合并
			...
			if (Dictionary.getSingleton().isStopWord(this.segmentBuff, result.getBegin(), result.getLength())) {
				...
				if (isChinese == 2) {
					String pinyin = PinyinHelper.convertToPinyinString(term, "", PinyinFormat.WITHOUT_TONE);
					result.setLexemeText(pinyin);
					result.setLexemeType(Lexeme.TYPE_PINYIN);
				} else if (isChinese == 3) {
					String pinyin_initial = PinyinHelper.getShortPinyin(term);
					result.setLexemeText(pinyin_initial);
					result.setLexemeType(Lexeme.TYPE_PINYIN);
					results.pollFirst();
					isChinese = 0;
					term = "";
				} else {
					term = String.valueOf(segmentBuff, result.getBegin(), result.getLength());
					result.setLexemeText(term);
				}
				break;
			}
		}
